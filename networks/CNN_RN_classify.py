...
def ConvolutionNetworks(no_filters=32,kernel_size=3, stride_size=1):
    def conv(model):
        model = Conv2D(no_filters, (3,3), strides=(1,1), activation='relu', input_shape=input_shape, data_format='channels_last')(model)
        model = MaxPooling2D()(model)
        model = BatchNormalization()(model)
        
        model = Conv2D(no_filters, (3,3), strides=(1,1), activation='relu')(model)
        model = MaxPooling2D()(model)
        model = BatchNormalization()(model)
...
def RelationNetworks(objects):
    ...
    # sum to make the network combinatorially generalizable
...
# Train Model
#history = RN_model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=batch_size, epochs=epochs, verbose=0, callbacks=[es, mc])
...
#print(y_test.shape)
#print('y_test:', y_test[1:15])


#print(y_test_pred.shape)
#print('y_test_pred:', y_test_pred[1:15])

#print('y_test_prob:', y_test_prob[1:15])
...
np.save('train_loss_CNN_RN',train_loss)
...
plt.title('Loss variation with epochs for CNN RN')
#plt.legend()
plt.show()
