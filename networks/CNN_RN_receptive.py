...
from keras.layers import Conv2D, Input, MaxPooling2D, AveragePooling2D, LeakyReLU
...
def ConvolutionNetworks(no_filters=32, kernel_size=3, stride_size=1):
    def conv(model):
        ...
        #model = Conv2D(no_filters, (3,3), strides=(stride_size,stride_size), activation='linear')(model) # d=5
        #model = Conv2D(no_filters, (5,5), strides=(stride_size,stride_size), activation='linear')(model) # d=4 
        model = Conv2D(no_filters, (7,7), strides=(stride_size,stride_size), activation='linear')(model) # d=3       
        model = LeakyReLU(alpha=leaky_slope)(model)
        model = MaxPooling2D()(model)
        model = BatchNormalization()(model)
...
np.save('train_loss_CNN_RN_receptive',train_loss_receptive)
